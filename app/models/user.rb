class User < ApplicationRecord
  ...
  def are_we_friends?(other_user_id)
    sql = "SELECT u.id, u.username,u.profile_pic_file_name, f.id AS friendship_id, f.status
           FROM users u 
           JOIN friendships f ON (f.user_id = #{self.id} AND f.friend_id = #{other_user_id}) OR (f.friend_id = #{self.id} AND f.user_id = #{other_user_id})
           WHERE u.id != #{self.id} AND f.status = 'friends'"  
    friends = User.find_by_sql(sql)
    return true if friends != [] || other_user_id == self.id
    false
  end

  def blocked?(other_user_id)
    blocked = self.blocked.any?{|blocked_person| blocked_person.id == other_user_id } 
    block_by = self.blocked_me.any?{|blocked_from| blocked_from.id == other_user_id}
    blocked = blocked or block_by
  ...
  def search(username)
    ...
    sql = "SELECT u.id, u.username, u.profile_pic_file_name
           FROM users u 
           WHERE u.id != #{self.id} AND u.username LIKE '#{username}%' AND NOT EXISTS(SELECT friendships.id FROM friendships WHERE (friendships.user_id = #{self.id} AND friendships.friend_id = u.id) OR (friendships.friend_id = #{self.id} AND friendships.user_id = u.id))"
    unexisting_friendships = User.find_by_sql(sql)
    sql = "SELECT u.id, u.username,u.profile_pic_file_name, f.id AS friendship_id, f.status
           FROM users u JOIN friendships f ON (f.user_id = #{self.id} AND f.friend_id = u.id) OR (f.friend_id = #{self.id} AND f.user_id = u.id)
           WHERE u.id != #{self.id} AND u.username LIKE '#{username}%' AND EXISTS(SELECT friendships.id FROM friendships WHERE (friendships.user_id = #{self.id} AND friendships.friend_id = u.id) OR (friendships.friend_id = #{self.id} AND friendships.user_id = u.id))
           AND CASE
           WHEN f.status = 'blocked' AND f.blocker != #{self.id}
           THEN false
           ELSE true
  ...
  def pending
    sql = "SELECT u.id, u.username,u.profile_pic_file_name, f.id AS friendship_id, f.status
           FROM users u 
           JOIN friendships f ON (f.friend_id = #{self.id} AND f.user_id = u.id)
           WHERE u.id != #{self.id} AND f.status = 'waiting'"  
